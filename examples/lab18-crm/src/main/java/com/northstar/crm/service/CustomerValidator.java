...
public class CustomerValidator {
    ...
    public void validateNew(Customer customer) {
        ...
        if (repository.existsById(customer.getCustomerId())) {
            throw BusinessException.conflict("duplicate customerId: " + customer.getCustomerId(), "lab-request-001");
        }
        if (repository.existsByEmail(customer.getEmail())) {
            throw BusinessException.conflict("duplicate email: " + customer.getEmail(), "lab-request-001");
        }
    }
